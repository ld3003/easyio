...
static rt_uint8_t tcpip_stack[ TCPIP_THREAD_STACKSIZE ];
static struct rt_thread tcpip_thread;
                            ...
                            int            prio)
{
    ...
    RT_DEBUG_NOT_IN_INTERRUPT;
		...
		//rt_kprintf("LWIP CREATE THREAD [%s]\r\n",name);
	
		
		if (strstr(name,"tcpip"))
		{
			//创建TCP/IP静态线程
			result = rt_thread_init(&tcpip_thread,
															name,
															thread,
															arg,
															(rt_uint8_t*)&tcpip_stack[0],
															sizeof(tcpip_stack),
															prio,
															20);
			if (result == RT_EOK)
			{
					rt_thread_startup(&tcpip_thread);
			}
...
static void __dbug_buf(unsigned char *buffer , int size)
{
	int i=0;
...
extern rt_err_t cmux_write(char port , unsigned char *data , int len);
u32_t sio_write(sio_fd_t fd, u8_t *buf, u32_t size)
{

	cmux_write(1,buf,(int)size);
	return size;
}

void sio_read_abort(sio_fd_t fd)
{
    DEBUGL->debug("read_abort\n");
}
