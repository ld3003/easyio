...
GSM0710_Frame *gsm0710_buffer_get_frame(GSM0710_Buffer *buf) {
  ...
  if (gsm0710_buffer_length(buf) >= length_needed) {
	...
	rt_kprintf("Out of memory, when allocating space for frame data.\n");
	frame->data_length = 0;
      }
    }
    // check FCS
    if (r_crctable[fcs^(*data)] != 0xCF) {
      rt_kprintf("Dropping frame: FCS doesn't match\n");
      destroy_frame(frame);
      buf->flag_found = 0;
      buf->dropped_count++;
      buf->readp = data;
      ...
      INC_BUF_POINTER(buf,data);
      if (*data != F_FLAG) {
	rt_kprintf("Dropping frame: End flag not found. Instead: %d\n", *data);
	destroy_frame(frame);
	buf->flag_found = 0;
	buf->dropped_count++;
	buf->readp = data;
